...
def _make_session(root: Path, name: str, *, opus: bool = True, transcript: bool = False) -> Path:
    d = root / name
    d.mkdir(parents=True, exist_ok=True)
    if opus:
        (d / "session.opus").write_bytes(b"")
    if transcript:
...
class TestPendingSessions:
    ...
    def test_session_with_opus_and_no_transcript_is_pending(self, tmp_path):
    ...
    def test_session_with_transcript_is_not_pending(self, tmp_path):
        _make_session(tmp_path, "20260101T100000", opus=True, transcript=True)
        assert pending_sessions(tmp_path) == []

    def test_session_without_opus_is_not_pending(self, tmp_path):
        _make_session(tmp_path, "20260101T100000", opus=False)
        assert pending_sessions(tmp_path) == []
    ...
    def test_mixed_sessions_only_pending_returned(self, tmp_path):
        _make_session(tmp_path, "20260101T100000", opus=True, transcript=False)
        _make_session(tmp_path, "20260101T110000", opus=True, transcript=True)
        _make_session(tmp_path, "20260101T120000", opus=True, transcript=False)
        result = pending_sessions(tmp_path)
        names = [d.name for d in result]
    ...
    def test_session_with_opus_is_detected_as_pending(self, tmp_path):
        """Sessions with session.opus (and no transcript) are pending."""
